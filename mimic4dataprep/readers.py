...
class Reader(object):
    def __init__(self, listfile_dir, listfile):
        self._listfile_dir = listfile_dir
        self._current_index = 0
        with open(os.path.join(self._listfile_dir, listfile), "r") as f_in:
            listfile_contents = f_in.readlines()
        self._listfile_header = listfile_contents[0]
        self._data = listfile_contents[1:]
...
class DecompensationReader(Reader):
    def __init__(self, listfile_dir, listfile):
        ...
        :param listfile_dir: Directory where the listfile is stored.
        :param listfile:    Listfile file name.
        """
        if listfile is None:
            raise ValueError("Listfile name must be provided.")
        Reader.__init__(self, listfile_dir, listfile)
        self._data = [line.split(',') for line in self._data]
        self._data = [(x, float(t), int(y)) for (x, t, y) in self._data]

    def _read_timeseries(self, ts_file_path, time_bound):
        ret = []
        with open(ts_file_path, "r") as tsfile:
...
class InHospitalMortalityReader(Reader):
    def __init__(self, listfile_dir, listfile, period_length=48.0):
