...
echo "  * Read 'min kept' and 'max kept' first. A cut point says nothing on its own; the"
echo "    surviving extremes say whether the rule reached the values it needed to."
echo "  * TAIL GAP: a 'none' cut means no detachment was found. That is the right answer for"
echo "    a continuous tail, but a single erroneous value between the body and a far one"
echo "    bridges the gap, so 'none' beside an absurd 'max kept' means the rule was defeated."
echo "  * ROBUST LOG SPREAD: z(min) and z(max) are the decision. Read down those columns for"
echo "    a --z that sits above the variables you know are right and below the ones you know"
echo "    are wrong, then rerun at that value and check what it removes."
echo "  * BELOW THE LOG: zeros and negatives, which neither rule can see. A small share is a"
echo "    sentinel for a measurement not taken; a large one is the variable itself."
echo "  * UNIT AUDIT: a conflict is read from the declared UNITNAME, so it is exact. The"
echo "    medians below it only say whether the conversion actually ran."
